...
def _turbine_state(w: float, rated_power: float) -> tuple[float, float, float]:
    ...
    match bisect.bisect_right(_REGIME_CUTS, w):
        case 0:
            return (0.0, 0.0, 90.0)
        case 1:
            return (rated_power * ((w - 3) / 9) ** 2.5, 10 + (w - 3) * 5, 5 + (12 - w) * 2)
        case _:
...
def fetch_nrel_wind_data(config=None):
    ...
    for year in years:
        ...
        params = {'api_key': NREL_API_KEY, 'wkt': f'POINT({lon} {lat})', 'attributes': attributes, 'names': str(year), 'utc': 'true', 'leap_day': 'false', 'interval': interval, 'email': email}
        try:
...
def prepare_val_dataset() -> None:
    global X_test, X_val, train_dataset, y_test, y_val
    ...
    mkdir(acc, auc, labels_true, out_dir, probs)
